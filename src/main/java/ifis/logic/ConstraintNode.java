...
public class ConstraintNode extends SHACLNode {
    ...
    protected void constructFromChildren() {
        ...
        /* for (var entry : getPShape().countMap.entrySet()) {
            ...
            // If all values are valid, the focus is valid, barring the count is in the cardinality range
            if (count == validCount) {
                                
                if (min != null && count < min.getMinCount()) continue;
                if (max != null && count > max.getMaxCount()) continue;
